#!/usr/bin/env Rscript
# sc_006.plot.pseudobulk.volcano.R —— pseudobulk DESeq2 火山图; 批量每对比一张
#   只需 *_pseudobulk_*_vs_*.csv (含 log2FoldChange/padj)。 Rscript sc_006.plot.pseudobulk.volcano.R
# ============================== CONFIG ==============================
LOGFC <- 1; PADJ <- 0.05; LABEL_N <- 12
UP_COL <- "red3"; DN_COL <- "blue3"; NS_COL <- "grey70"
# ===================================================================
suppressPackageStartupMessages(library(optparse))   # 只加载轻库; 重库延后, 加快 --help/usage
EX <- "
示例:
  Rscript sc_006.plot.pseudobulk.volcano.R --outdir results   # 对每个 *_pseudobulk_*_vs_*.csv 各出一张
  Rscript sc_006.plot.pseudobulk.volcano.R --input myrun_pseudobulk_Disease_vs_Control.csv
只需 CSV(含 log2FoldChange/padj)。细调 LOGFC/PADJ/LABEL_N/配色: 改脚本顶部 CONFIG 块。
"
parser <- OptionParser(option_list=list(
  ...
  description="sc_006.plot.pseudobulk.volcano.R — pseudobulk DESeq2 火山图, 批量每对比一张", epilogue=EX)
...
if (!length(files)) stop("找不到 *_pseudobulk_*_vs_*.csv (先跑 --do_pseudobulk)")
...
for (f in files) {
  ...
  d <- d[!is.na(d$padj), ]
  d$sig <- "NS"
  d$sig[d$log2FoldChange> LOGFC & d$padj<PADJ] <- "Up"
  d$sig[d$log2FoldChange< -LOGFC & d$padj<PADJ] <- "Down"
  lab <- rbind(d %>% filter(sig=="Up") %>% arrange(desc(log2FoldChange)) %>% head(LABEL_N),
               d %>% filter(sig=="Down") %>% arrange(log2FoldChange) %>% head(LABEL_N))
  p <- ggplot(d, aes(log2FoldChange, -log10(padj+1e-300), color=sig)) +
    ...
    labs(title=paste0("Pseudobulk (DESeq2) volcano: ", A, " vs ", B),
  ...
  out <- file.path(outdir, sprintf("%s_plot_pseudobulk_volcano_%s_vs_%s.png", prefix, A, B))
