#!/usr/bin/env Rscript
# sc_006.plot.deg.heatmap.R —— DEG top 基因 × 细胞 热图 (A/B 两组); 批量每对比一张
#   需 rds + ident。 Rscript sc_006.plot.deg.heatmap.R
# ============================== CONFIG ==============================
TOPN <- 25; PADJ <- 0.05; LOGFC <- 0.25; CELLS_PER_GRP <- 200
# ===================================================================
suppressPackageStartupMessages(library(optparse))   # 只加载轻库; 重库延后, 加快 --help/usage
EX <- "
示例:
  Rscript sc_006.plot.deg.heatmap.R --outdir results     # 对每个 *_DEG_*_vs_*.csv 各出一张(需 rds)
  Rscript sc_006.plot.deg.heatmap.R --rds obj.rds --input myrun_DEG_Disease_vs_Control.csv
细调 TOPN/PADJ/LOGFC/每组抽样细胞 CELLS_PER_GRP: 改脚本顶部 CONFIG 块。
"
parser <- OptionParser(option_list=list(
  ...
  description="sc_006.plot.deg.heatmap.R — DEG top 基因 × 细胞 热图(A/B 两组), 批量每对比一张", epilogue=EX)
...
for (f in files) {
  ...
  if (!length(genes)) { cat("[skip] 无显著基因:", f, "\n"); next }
  grps <- if (toupper(B)=="ALL") levels(Idents(obj)) else c(A,B)
  md <- obj@meta.data; md$.cell <- rownames(md)
  set.seed(42)
  keep <- md %>% filter(.data[[ic]] %in% grps) %>% group_by(.data[[ic]]) %>%
    slice_sample(n=CELLS_PER_GRP) %>% pull(.cell)
  so <- obj[, keep]; Idents(so) <- ic
  ...
  p <- DoHeatmap(so, features=genes, group.by=ic, size=4.5, angle=0) +
    labs(title=paste0("DEG heatmap: ", A, " vs ", B)) +
  ...
  out <- file.path(outdir, sprintf("%s_plot_deg_heatmap_%s_vs_%s.png", prefix, A, B))
  h <- max(6, length(genes)*0.22)
  ggsave(out, p, width=10, height=h, dpi=300, limitsize=FALSE); ggsave(sub("\\.png$",".pdf",out), p, width=10, height=h, limitsize=FALSE)
  cat("[saved]", out, "\n")
}
